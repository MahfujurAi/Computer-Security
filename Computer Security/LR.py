...
#grid_search = GridSearchCV(model1, param_grid, cv=5, scoring=["accuracy", "f1_macro", "roc_auc"])
...
for metric, value in model.best_score_.items():
    print(f"\t{metric}: {value}")
print("\nBest hyperparameters:", model.best_params_)

# Get and analyze feature importances
feature_importances = model.best_estimator_.coef_[0]
important_features = [i for i, v in enumerate(feature_importances) if abs(v) > 0.1]
print("\nImportant features:", [feature_names[i] for i in important_features])

# Analyze model performance with different metrics
